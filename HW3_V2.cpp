...
int main(){
  ...
  //  fileOut3 << tree[i].getTreeLevel() <<"\t"<<tree[i].getTreeNode() <<endl;
  //code geration
                    ...
                    if(type1.compare(type2)==0){
                       ...
                       if(type1.compare("int")==0){
                          ...
                          if(expr1Op.compare("/")==0)
                             fileOut2 << "sdiv i32 ";
                          if(expr1Op.compare(">")==0)
                             fileOut2 << "icmp sgt i32 ";
                          if(expr1Op.compare("<")==0)
                             fileOut2 << "icmp slt i32 ";
                          if(expr1Op.compare(">=")==0)
                             fileOut2 << "icmp sge i32 ";
                          if(expr1Op.compare("<=")==0)
                             fileOut2 << "icmp sle i32 ";
                       }else{
                          if(expr1Op.compare("+")==0)
                          ...
                          if(expr1Op.compare(">")==0)
                             fileOut2 << "fcmp ogt double ";
                          if(expr1Op.compare("<")==0)
                             fileOut2 << "fcmp olt double ";
                          if(expr1Op.compare(">=")==0)
                             fileOut2 << "fcmp oge double ";
                          if(expr1Op.compare("<=")==0)
                             fileOut2 << "fcmp ole double ";
                       }
                       fileOut2 << "%" << exprReg << ", %" << expr1IdRegNum << endl;
/////
                       if(expr1Op.compare(">")==0 || expr1Op.compare("<")==0 || expr1Op.compare("<=")==0 || expr1Op.compare(">=")==0){
                          fileOut2<<"%"<<itos(tempRegNum)<<" = zext i1 %"<<itos(tempRegNum-1)<<" to i32"<<endl;
                          tempRegNum++;
                          string i32Id;
                          for(int q=0;q<symbolTable.size();q++){
                             if(symbolTable[q].getType().compare("int")==0)
                                 i32Id = symbolTable[q].getSymbol();
                          }
                          return itos(tempRegNum-1)+" "+i32Id;
                       }
                       else
//// 
